...
public class DisplayUtills {
   ...
   public static void initGL() {
	  ...
	  int startHeight = 25;
	  int height = (int) g.generateHeight(0, 0);
	  int cx = 0;
	  int cy = 0;
	  int cz = 0;
	  int ch = 0;
	  ...
	  for (int i2 = 0; i2 < chunks.length; i2++) {
			...
			if (y > height) {
			   ...
			   if (z > 34) {
				   break;
					...
					height = (int) g.generateHeight(x + (cx *35), z + (cz*35));
				}
		 ...
		 chunks[i2] = new Chunk(Blocks, cx, cy, cz, i2);
		 cy++;
   ...
   public static void renderGL() {
	  ...
	  for (Chunk chunk : ativeChunks) {
		 ...
		 float YH = 0;
		 //BlockDirt b = new BlockDirt((int)(posX-BX), (int)(YH - posY),(int)( posZ-BZ));
		 //b.drawBlock( (int)(posX-BX), (int)(YH - posY),(int)( posZ-BZ),1);
   ...
   public static void placeBlock(BlockLocation bl, Block b) {
	   chunks[ativeChunks[bl.i].AN].Blocks[bl.bi] = b;
   }
   public static void removeBlock() {
	   
   }
   public static BlockLocation getBlockLocation(int x, int y, int z) {
	   
   int i = 0;
   int i2 = 0;
   boolean Break = false;
	   while(i < ativeChunks.length ) {
		    i2 = 0;
		   while(i2 < ativeChunks[i].Blocks.length) {
			   Block b = ativeChunks[i].Blocks[i2];
			   if(b != null) {
			   if(ativeChunks[i].getX() * 35 + b.getX() == x && ativeChunks[i].getZ() * 35 + b.getZ() == z){
				   Break = true;
				   break;
	...
	return new BlockLocation(ativeChunks[i].Blocks[i2].getX(), ativeChunks[i].Blocks[i2].getY(), ativeChunks[i].Blocks[i2].getZ(), i, i2);
